...
def build_lesson_prompt(user_id: int, folder_name: str) -> str | None:
    ...
    try:
        ...
        prompt += (
            ...
            "a simpler analogy, or break it into even smaller steps.\n"
            "8. BEFORE completing the section, you MUST do a PRACTICE ROUND:\n"
            "   a) Look through the source material for any exercises, problems, practice questions, or worked examples.\n"
            "   b) If exercises exist in the source material, present them to the student one at a time and have them solve each one. "
            "Guide them through it if they get stuck, but let them try first.\n"
            "   c) If NO exercises exist in the source material, create 2-3 practice problems yourself that test the key concepts "
            "of this section. Make them progressively harder — start easy to build confidence, then challenge them.\n"
            "   d) If the student gets a problem wrong, explain what went wrong, identify the concept they're weak on, "
            "and tell them clearly: 'You should review [specific topic] more — this is an area to focus on.'\n"
            "   e) Only after the student has attempted the practice problems should you consider the section complete.\n"
